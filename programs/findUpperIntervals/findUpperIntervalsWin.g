...
findUpperIntervals:=function(args)
    # findUpperIntervals:=function([n1, n2, minsz, maxsz, abelian, solvable, core, norepeats])
    ...
    # updated: 2011.05.14
    #
    local i, j, k, m, r, s, pathname, filename, datfile, outfile, logfile, first, incls, newcnt, maxsize, indx,
          G, e, cc, numcc, numsg, lats, latsize, obtained, intSubs, H, perm, t, prefix, ignore, answer, pgroup,
          n1, n2, minsz, maxsz, abelian, solvable, core, cores, corefree, nargin, norepeats, BETA, VERBOSE;
    
    BETA:=true;  # set this to false when we're done with unit testing
    VERBOSE:=true;
    
    nargin := Length(args);
    if nargin < 9 then norepeats:=9; else norepeats:=args[9]; fi;
    ...
    filename := Concatenation(pathname, prefix, "-", timeStamp(), ".out");
    ...
    filename := Concatenation(pathname, prefix, "-", timeStamp(), ".dat");
    ...
    # This one stores output in format that will be read in by BatchCovers2Hasse.m Matlab routine:
    filename := Concatenation(pathname, prefix, "-", timeStamp(), ".log");
    ...
    PrintTo(outfile, "by the GAP function:  findUpperIntervals( ", args, " ); \n\n");
    PrintTo(datfile, "This data file was created on ", getSystemDate(), "\n");
    PrintTo(datfile, "by the GAP function:  findUpperIntervals( ", args, " ); \n\n");
    PrintTo(logfile, "This log file was created on ", getSystemDate(), "\n");
    PrintTo(logfile, "by the GAP function:  findUpperIntervals( ", args, " ); \n\n");
    ...
    for i in [n1..n2] do
        ...
        AppendTo(logfile, "Checking the ", numsg, " group(s) of order |G| = ", i, "...  (", newcnt, " intervals so far) \n");
        ...
        for j in [1..numsg] do
            ...
            elif pgroup=-1 then  # pgroup=-1 means include ONLY p-groups in the search
                ignore:=true;
            fi;
            if not ignore then
                if IsAbelian(G) then
                ...
                elif abelian=-1 then  # abelian=-1 means include ONLY abelian groups in the search
                    ignore:=true;
                elif solvable=0 and IsSolvable(G) then
                    ignore:=true;
                elif solvable=-1 then  # solvable=-1 means include ONLY solvable groups in the search
                    ...
                    if corefree then
                        ...
                        if minsz <= latsize and latsize <= maxsz then
                            ...
                            if not obtained then
                                ...
                                AppendTo(outfile, StructureDescription(G), "H = CC[", k, "] = ", StructureDescription(H));
                                AppendTo(outfile, "  Index: ", indx, ", ",  incls, "\n"); 
                                AppendTo(datfile, incls,"; [H,G] = [CC[",k,"], SmallGroup(", i, ",", j, ")] = ",
                                        "[ ", StructureDescription(H), ",  ", StructureDescription(G), " ]  Index: ", indx);
                                if VERBOSE then
